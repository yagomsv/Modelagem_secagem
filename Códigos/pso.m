...
MSE = @(n, ft, tx) My_ANN(n, ft, tx); % Função mse
RMSE = @(n, ft, tx) My_ANN(n, ft, tx); % Função rmse
R2 = @(n, ft, tx) My_ANN(n, ft, tx); % Função R2
...
c1 = 1.4;      % Coeficiente de aceleração pessoal
c2 = 1.4;      % Coeficiente de aceleração global
...
for it = 1:MaxIt  
   for i =1:nPop
    ...
    particle(i).rmse = RMSE(particle(i).Positionn, particle(i).Positionft, particle(i).Positiontx);
    particle(i).R2 = R2(particle(i).Positionn, particle(i).Positionft, particle(i).Positiontx);
    % Update da melhor posição
    if particle(i).mse < particle(i).Best.mse
       ...
       particle(i).Best.rmse = particle(i).rmse;
       particle(i).Best.R2 = particle(i).R2;
      % Update o melhor global
      if particle(i).Best.mse < GlobalBest.mse
        GlobalBest = particle(i).Best;
      end
